...
class MailController extends Controller
{
    ...
    public function add($account, $com_id)
    {
        try {
            ...
            $to              = $re[0]['email'];
            if ($to == "") {
                $this->feedback(C('PERSON_SEND_MAIL_NO_DESTINATION_ERROR_CODE'), 'No destination', null);
                //exit("");
            } else {
                ...
                if ($this->sendMail($to, $title, $content, $attachment)) {
                    ...
                    $this->feedback(C('PERSON_SEND_MAIL_ERROR_CODE'), 'email failed', null);
                    //exit("");
                }
            }
        } catch (\Exception $e) {
    ...
    public function check($account, $com_id)
    {
        ...
        if ($judge == 0) { 
            $this->feedback(C('PERSON_SEND_MAIL_NO_DATA_CODE'), "not write", null);
        } else {
            ...
            if (count($result) == 0) {
                ...
                if (($now - $result[0]['time']) > $between) {
                    ...
                    $this->feedback(C('PERSON_SEND_MAIL_HAVE_SENT_CODE'), "have sent", null);
                }
            }
        }
    }
